...
export default function AdminAddSku() {
  ...
  const loadData = async () => {
    // setLoading(true);
    try {
      const apiCalls = [apiService.fetchAllCollection()];
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          switch (index) {
            case 0:
              // console.log("checking collection", result);
              // setAllcollectiondata(result.value);

              // Check if result.value is an array before setting it
            if (Array.isArray(result.value)) {
              setAllcollectiondata(result.value); // Set data if it's an array
            } else {
              console.error("Expected array but got something else:", result.value);
              setAllcollectiondata([]); // Set an empty array if result is not an array
            }
  ...
  const updateDiamond = (index, field, value) => {
    ...
    //   const diamondWeight =
    //       field === "DiamondWeight" ? value : updatedDiamond.DiamondWeight || "0";
    //   const totalWeight = Number(diamondPieces) * Number(diamondWeight);
    //   updatedDiamond.TotalDiamondWeight = String(totalWeight.toFixed(2));
    // }
    // if (field === "DiamondShape") {
    //   console.log("updatedDiamondupdatedDiamond : ",updatedDiamond)
    ...
    // }
    // if (field === "DiamondClarity" || field === "DiamondShape") {
    //   const singleTemplateData = defaultTemplateData.DiamondSizeWeightRates.find((item) => (item.DiamondClarity == value && item.DiamondShape == newSku.Diamonds[index].DiamondShape));
    //   singleTemplateData ? updatedDiamond.DiamondRate = String(singleTemplateData.DiamondPurchaseRate) : null;
    // }
    // if (field === "DiamondColour" || field === "DiamondClarity" || field === "DiamondShape") {
    //   const singleTemplateData = defaultTemplateData.DiamondSizeWeightRates.find((item) => (item.DiamondColor == value && item.DiamondShape == newSku.Diamonds[index].DiamondShape && item.DiamondClarity == newSku.Diamonds[index].DiamondClarity));
    //   singleTemplateData ? updatedDiamond.DiamondRate = String(singleTemplateData.DiamondPurchaseRate) : null;
    // }
    // if (field === "Sleve" || field === "DiamondColour" || field === "DiamondClarity" || field === "DiamondShape") {
    ...
    if (
      ...
      const singleTemplateData =
        defaultTemplateData.DiamondSizeWeightRates.find(
          (item) =>
            item.DiamondWeight ==
              (field === "DiamondWeight"
                ? value
                : newSku.Diamonds[index].DiamondWeight) &&
            item.Sleve ==
              (field === "Sleve" ? value : newSku.Diamonds[index].Sleve) &&
            item.DiamondShape ==
              (field === "DiamondShape"
                ? value
                : newSku.Diamonds[index].DiamondShape) &&
            (field === "DiamondClarity"
              ? value
              : item.DiamondClarity ==
                (field === "DiamondClarity"
                  ? value
                  : newSku.Diamonds[index].DiamondClarity)) &&
            item.DiamondSize ==
              (field === "DiamondSize"
                ? value
                : newSku.Diamonds[index].DiamondSize)
        );
      singleTemplateData
        ? (updatedDiamond.DiamondRate = String(
            singleTemplateData.DiamondPurchaseRate
          ))
        : null;
  ...
  const handleEditData = (data) => {
    ...
    console.log('checking sku data ', data);
                ...
                <div className="adminCategoryAddCategoryInnerBox">
                  <div className="adminSkuAddSkuInnerBox">
                    ...
                    <div className="adminSkuAddSkuInnerItemsBox">
                      ...
                      <select
                        name="CollectionId"
                        value={newSku.CollectionId}
                        onChange={handleNewSkuChange}
                      >
                        <option value="">Collection</option>
                        {allcollectiondata.map((x, y) => {
                          return (
                            <option key={y} value={parseInt(x.Collection.Id)}>
                  ...
                  <div className="adminSkuAddSkuInnerItemsStoneBox">
                    {newSku.Diamonds.map((diamond, index) => (
                      <div
                        className="adminSkuAddSkuInnerItemsStoneInnerBox"
                        style={{
                          display: "grid",
                          gridTemplateColumns: {
                            // xs: 'repeat(2, 1fr)',
                      ...
                      >
                        ...
                        <div className="adminSkuAddSkuInnerItemsBox">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateDiamond(index, "Sleve", e.target.value)
                            }
                          />
                        </div>
                        <div className="adminSkuAddSkuInnerItemsBox">
